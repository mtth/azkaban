...
Usage:
  azkaban PROJECT run [-m MODE] ([-u USER] URL | -a ALIAS) FLOW [JOB ...]
  azkaban PROJECT upload ([-u USER] URL | -a ALIAS) [SCRIPT | -z ZIP]
  azkaban PROJECT build [-oz ZIP] [SCRIPT]
  azkaban PROJECT list [-fp] [SCRIPT]
  azkaban PROJECT view [SCRIPT] JOB
...
Examples:
  ...
  azkaban my_project upload http://url.to.azkaban
...
def main(args):
  ...
  :param args: command line arguments dictionary (as parsed by docopt)
  ...
  try:
    ...
    if args['build']:
      project = get_project(name, args['SCRIPT'])
      project.build(
        args['--zip'] or '%s.zip' % (name, ),
